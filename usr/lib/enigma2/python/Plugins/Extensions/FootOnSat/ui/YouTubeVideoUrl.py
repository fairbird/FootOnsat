...
def create_priority_formats():
	...
	try:
		if hasattr(config.plugins, "FootOnSat"):
			itag = config.plugins.FootOnSat.best_resolution.value
		else:
			itag = "22"
	except (AttributeError, KeyError):
		itag = "22"
	video_formats = (
		('17', '91', '13', '151', '160', '269'),
		('5', '36', '92', '132', '133', '229'),
		('18', '93', '34', '6', '134', '230'),
		('35', '59', '78', '94', '135', '212', '231'),
		('22', '22', '136', '232', '298'),
		('37', '37', '137', '233', '299', '138', '234', '266', '303'),
		('38', '38', '138', '234', '266', '303')
	)
	PRIORITY_VIDEO_FORMAT = video_formats[2]
	for fmt_group in video_formats:
		if fmt_group[0] == itag:
			PRIORITY_VIDEO_FORMAT = fmt_group
			break
...
class YouTubeVideoUrl():
	...
	def _real_extract(self, video_id, yt_auth):
		...
		if PRIORITY_VIDEO_FORMAT[0] != config.plugins.FootOnSat.best_resolution.value:
			create_priority_formats()
