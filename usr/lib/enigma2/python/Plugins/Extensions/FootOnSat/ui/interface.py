...
debug_LiveTicker = config.plugins.FootOnSat.debug_LiveTicker.value
...
class LiveTickerSummary(Screen):
	if DreamOS():
		skin = """
		<screen name="LiveTickerSummary" position="0,0" size="132,64">
			<widget source="entry" render="Label" position="center,center" size="132,64" font="Regular;25" />
		</screen>"""
	else:
		skin = """
		<screen name="LiveTickerSummary" position="fill">
			<widget source="entry" render="Label" position="0,60" size="e,45" font="Regular;25" halign="center" valign="center" />
		</screen>"""
        ...
        def selectionChanged(self, name):
                if debug_LiveTicker: logdata("LiveTickerSummary", "Summary selectionChanged received: %s" % str(name))
                items = [str(x) for x in self.parent.live_matches] if hasattr(self.parent, "live_matches") and self.parent.live_matches else []
                text = "  ***  ".join(items) if items else str(title309)
                self["entry"].setText(text)


class LiveTicker(Screen):
        def __init__(self, session):
                if debug_LiveTicker: logdata("LiveTicker", "Initializing LiveTicker Screen")
                desk = getDesktop(0)
                width = desk.size().width()
                if isUHD():
                        self.skin = """<screen name="LiveTicker" position="0,500" size="%d,200" backgroundColor="#16000000" flags="wfNoBorder" zPosition="999">
                                        <widget name="ticker" position="0,0" size="15000,200" font="Regular;90" valign="center" halign="left" transparent="1" foregroundColor="#ffffff" noWrap="1" />
                                        </screen>""" % width
                else:
                        self.skin = """<screen name="LiveTicker" position="0,980" size="%d,100" backgroundColor="#16000000" flags="wfNoBorder" zPosition="999">
                                        <widget name="ticker" position="0,0" size="15000,100" font="Regular;40" valign="center" halign="left" transparent="1" foregroundColor="#ffffff" noWrap="1" />
                                        </screen>""" % width
                Screen.__init__(self, session)
                self["ticker"] = Label(_("%s") % title308)
                self["actions"] = ActionMap(["SetupActions", "ColorActions"], {"cancel": self.close,"ok": self.close}, -1)
                self.live_matches = []
                self.onChangedEntry = []
                self.scroll_text = ""
                self.ticker_x = width
                ...
                self.timer = eTimer()
                try:
                        self.timer_conn = self.timer.timeout.connect(self.scrollTicker)
                except:
                        self.timer.callback.append(self.scrollTicker)
                self.fetchLive()

        def selectionChanged(self):
                if self.live_matches:
                        name = self.live_matches[0]
                else:
                        name = "-"
        ...
        def fetchLive(self):
                if debug_LiveTicker: logdata("LiveTicker", "fetchLive initiated (ESPN).")
                today_str = date.today().strftime("%Y%m%d")
                espn_url = "https://site.api.espn.com/apis/site/v2/sports/soccer/all/scoreboard?dates={0}".format(today_str)
                if debug_LiveTicker: logdata("LiveTicker", "espn_url: %s" % str(espn_url))
                head = {
                        b"User-Agent": [b"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/117.0 Safari/537.36"],
                        b"Accept": [b"application/json"],
                }
                if PY3:
                        try:
                                sniFactory = WebClientContextFactory(espn_url)
                        except Exception as e:
                                if debug_LiveTicker: logdata("LiveTicker", "SNI factory error: %s" % str(e))
                                self.parseData([])
                                return
                        d = getPage(str.encode(espn_url), contextFactory=sniFactory, timeout=15, headers=head)
                        d.addCallback(self._gotEspnData)
                        d.addErrback(self.dataError)
                else:
                        def _py2_fetch():
                                try:
                                        sr = requests.Session()
                                        r = sr.get(espn_url, timeout=15, headers={"User-Agent": "Mozilla/5.0", "Accept": "application/json"}, verify=False)
                                        return r.content if r.status_code == 200 else None
                                except Exception as e:
                                        if debug_LiveTicker: logdata("LiveTicker", "Py2 fetch error: %s" % str(e))
                                        return None
                        deferToThread(_py2_fetch).addCallback(self._gotEspnData).addErrback(self.dataError)

        def _gotEspnData(self, raw):
                if not raw:
                        if debug_LiveTicker: logdata("LiveTicker", "ESPN returned empty response")
                        self.parseData([])
                        return
                if debug_LiveTicker: logdata("LiveTicker", "ESPN raw response (first 500 chars): %s" % str(raw[:500]))
                try:
                        data = json.loads(raw.decode('utf-8') if isinstance(raw, bytes) else raw)
                except Exception as e:
                        if debug_LiveTicker: logdata("LiveTicker", "JSON parse error: %s" % str(e))
                        self.parseData([])
                        return
                all_events = data.get("events", [])
                if debug_LiveTicker: logdata("LiveTicker", "ESPN total events returned: %d" % len(all_events))
                states_seen = []
                matches = []
                for ev in all_events:
                        try:
                                ...
                                t1 = home.get("team", {}).get("displayName", "")
                                t2 = away.get("team", {}).get("displayName", "")
                                s1 = home.get("score", "")
                                s2 = away.get("score", "")
                                clock = status_type.get("shortDetail", "") or comp.get("status", {}).get("displayClock", "")
                                if t1 and t2:
                                        matches.append({"team1": t1, "team2": t2, "team1_score": s1 if s1 != "" else "0", "team2_score": s2 if s2 != "" else "0", "match_status": clock if clock else "0'"})
                        except Exception:
                                continue
                if debug_LiveTicker: logdata("LiveTicker", "ESPN event states seen: %s" % str(states_seen))
                if debug_LiveTicker: logdata("LiveTicker", "ESPN live matches found: %d" % len(matches))
                self.parseData(matches)

        def fetchLiveTicker(self):
                self.fetchLive()

        def parseData(self, data):
                if debug_LiveTicker: logdata("LiveTicker", "Parsing live data... data received: %s" % str(data))
                new_matches = []
                if data:
                        for match in data:
                                if debug_LiveTicker: logdata("LiveTicker", "Inspecting match item: %s" % str(match))
                                if isinstance(match, (list, tuple)) and len(match) > 7:
                                        match_name = compat_str(match[0])
                                        s1 = match[5]
                                        s2 = match[6]
                                        status = compat_str(match[7])
                                        if debug_LiveTicker: logdata("LiveTicker", "Match tuple parsed -> Name: %s, S1: %s, S2: %s, Status: %s" % (match_name, str(s1), str(s2), status))
                                        if s1 is not None and s2 is not None and str(s1) != "" and str(s2) != "":
                                                teams = re.split(r'\s+(?:vs\.|vs|v\.|v|VS|Vs|VS\.)\s+|\s+-\s+', match_name, flags=re.IGNORECASE)
                                                if len(teams) == 2:
                                                        new_matches.append("%s %s-%s %s (%s)" % (teams[0].strip(), s1, s2, teams[1].strip(), status))
                                                else:
                                                        new_matches.append("%s %s-%s (%s)" % (match_name, s1, s2, status))
                                        elif status and status.lower() not in ["", "ft", "finished", title125.lower(), title214.lower(), title152.lower()]:
                                                new_matches.append("%s [%s]" % (match_name, status))
                                        ...
                                        if t1 and t2:
                                                score = "%s-%s" % (s1, s2) if (s1 is not None and str(s1) != "" and s2 is not None and str(s2) != "") else status
                                                new_matches.append("%s %s %s" % (t1, score, t2))
                if new_matches:
                        body = "    ***    ".join(new_matches)
                        sep = "    ***    "
                        single_str = body + sep
                        new_text = single_str * 50
                        if new_text != self.scroll_text:
                                self.live_matches = new_matches
                                self.scroll_text = new_text
                                self.selectionChanged()
                                self["ticker"].setText(single_str)
                                if self["ticker"].instance:
                                        self["ticker"].instance.calculateSize()
                                        self.single_text_width = self["ticker"].instance.size().width()
                                self["ticker"].setText(new_text)
                                desk = getDesktop(0)
                                width = desk.size().width()
                                if self.ticker_x <= 0 or self.ticker_x >= width:
                                        self.ticker_x = width
                                self.timer.stop()
                        ...
                        self.selectionChanged()
                        body = str(title309)
                        self.scroll_text = body
                        self["ticker"].setText(body)
                        if self["ticker"].instance:
                                char_w = 60 if isUHD() else 30
                                text_width = len(body) * char_w
                                desk = getDesktop(0)
                                width = desk.size().width()
                                center_x = (width - text_width) // 2
                                if center_x < 0:
                                        center_x = 0
                                self["ticker"].instance.move(ePoint(int(center_x), 0))

        def dataError(self, error):
                if debug_LiveTicker: logdata("LiveTicker", "Data error callback triggered: %s" % str(error))
                self["ticker"].setText("Error fetching data")

        def scrollTicker(self):
                if self.scroll_text and self["ticker"].instance:
                        self.ticker_x -= 6
                        if self.single_text_width > 0:
                                if self.ticker_x <= -self.single_text_width:
                                        self.ticker_x += self.single_text_width
                        if debug_LiveTicker: logdata("LiveTicker", "scrollTicker running -> ticker_x: %d | text_width: %d" % (self.ticker_x, self.single_text_width))
                        try:
                                self["ticker"].instance.move(ePoint(int(self.ticker_x), 0))
                        except Exception as e:
                                if debug_LiveTicker: logdata("LiveTicker", "scrollTicker move error: %s" % str(e))
                        self.timer.start(20, True)

        def createSummary(self):
                return LiveTickerSummary
